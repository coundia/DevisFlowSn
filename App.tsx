...
const App: React.FC = () => {
  ...
  return (
    <div className={`${isDark ? 'dark' : ''} min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col font-inter antialiased transition-colors duration-500`}>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="hidden xs:block">
            ...
            <div className="flex items-center gap-2 mt-1.5">
               <p className="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest">Facturation Intelligence</p>
               {!isOnline && (
        ...
        <div className="flex items-center gap-3 sm:gap-5">
          <div className="hidden md:flex items-center gap-2.5 px-3.5 py-2 bg-slate-100/50 dark:bg-slate-800/40 rounded-xl border border-slate-200/60 dark:border-slate-700/50">
            ...
            <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">
              Live Save: {lastSaved.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
            </span>
          </div>
          ...
          <button 
            ...
            className="p-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-white dark:hover:bg-slate-700 hover:shadow-sm transition-all border border-transparent dark:border-slate-700"
          >
          ...
          >
            ...
            <span className="hidden sm:inline uppercase tracking-widest text-[11px]">{isDownloading ? 'Working...' : 'Export PDF'}</span>
      ...
      <main className="flex-1 max-w-[1600px] mx-auto w-full p-4 lg:p-8 grid grid-cols-1 lg:grid-cols-12 gap-10 relative">
        <aside className="lg:col-span-1 hidden lg:flex flex-col gap-5 sticky top-32 no-print h-fit">
          ...
          <button 
            ...
            className="w-20 h-20 rounded-3xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-400 hover:text-rose-500 hover:border-rose-100 dark:hover:border-rose-900 flex flex-col items-center justify-center transition-all hover:-translate-y-1"
          >
          ...
          <div className="lg:hidden flex overflow-x-auto gap-3 pb-4 scrollbar-hide no-print">
            {steps.map(s => (
              <button 
                ...
                className={`px-6 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest shrink-0 border transition-all ${activeStep === s.id ? 'bg-indigo-600 text-white border-indigo-600 shadow-lg shadow-indigo-100 dark:shadow-none' : 'bg-white dark:bg-slate-900 text-slate-500 border-slate-100 dark:border-slate-800'}`}
              >
                {s.label}
              </button>
            ))}
          ...
          <div className="flex justify-between items-center pt-6 no-print">
            ...
            <div className="bg-slate-950 dark:bg-slate-900 text-white px-7 sm:px-10 py-4 rounded-[2.5rem] flex items-center gap-6 sm:gap-10 shadow-2xl border border-white/10">
               <div className="flex flex-col items-end">
                 <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest leading-none mb-1.5">Grand Total</span>
               ...
               >
                 {activeStep < 3 ? 'Next' : 'Review'} <ChevronRight className="w-4 h-4" />
               </button>
            </div>
          </div>
        </section>
        ...
        <section className={`lg:col-span-6 sticky top-28 h-fit ${activeTab === 'preview' ? 'block' : 'hidden lg:block'}`}>
           <div className="bg-white dark:bg-slate-900 rounded-[3rem] shadow-[0_40px_80px_-20px_rgba(0,0,0,0.15)] dark:shadow-none overflow-hidden border border-slate-100 dark:border-slate-800 relative group">
              ...
              <div className="bg-slate-950 dark:bg-slate-800 text-white px-8 py-3.5 flex justify-between items-center no-print">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-[0.2em]">HD Canvas Render</span>
                </div>
                <div className="flex items-center gap-4">
                  <span className="text-[9px] text-slate-400 font-black uppercase tracking-widest">ISO 216 / A4</span>
                  {activeTab === 'preview' && (
                    <span className="text-[9px] bg-indigo-600 px-3 py-1 rounded-full font-black uppercase tracking-tighter">1:1 Zoom</span>
                  )}
                </div>
              </div>
              <div id="invoice-preview-capture" className="bg-slate-100 dark:bg-slate-950 p-2 sm:p-6 md:p-12 overflow-hidden transition-colors duration-500">
      ...
      {isDownloading && (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-md z-[200] flex items-center justify-center no-print animate-in fade-in duration-300">
          <div className="bg-white dark:bg-slate-900 p-12 rounded-[3rem] shadow-2xl flex flex-col items-center gap-6 max-w-sm text-center border border-slate-100 dark:border-slate-800">
            ...
            <div>
              <h4 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tight">Exporting Document</h4>
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-2 font-medium">Applying styles and generating high-resolution PDF assets...</p>
            </div>
          </div>
        </div>
      )}
